<?php
	if (isset($_GET['id'])){
		...
		foreach($articles['events'] as $article) {
			...
			echo "<span class=\"naslov\"><a class=\"naslov\" href=\"dogadjaji/$y/$naslov\">".$article->get('title')."</a></span>";
		...
		if ($broj_strana!=1 && $broj_strana!=0){
			...
			if ($strana!=1) {
				$a=$strana-1;
				echo "<a href='dogadjaji/strana/$a'><img class=\"strelica\" src=\"images/left_arrow.png\"/></a>";
			}
			else{echo '';}
			...
			if ($strana!=$broj_strana) {
				$b=$strana+1;
				echo "<a href='dogadjaji/strana/$b'><img class=\"strelica\" src=\"images/right_arrow.png\"/></a>";
			}
			else{echo '';}
			echo '</span>';
		}
