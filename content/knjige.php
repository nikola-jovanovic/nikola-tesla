<?php
	if (isset($_GET['id'])){
		...
		if ($_COOKIE["slova"]=="cirilica") {
			echo '<h1>Књиге о Тесли</h1>';
		}
		else{
			echo '<h1>Knjige o Tesli</h1>';
		}
		...
		foreach($articles['books'] as $article) {
			$y = $article->get('id');
			$naslov = str_replace(' ','-',$article->get('title'));
			echo "<span class=\"naslov\"><a class=\"naslov\" href=\"knjige/$y/$naslov\">".$article->get('title')."</a></span>";
		...
		if ($broj_strana!=1 && $broj_strana!=0){
			...
			if ($strana!=1) {
				$a=$strana-1;
				echo "<a href='knjige/strana/$a'><img class=\"strelica\" src=\"images/left_arrow.png\"/></a>";
			}
			else{echo '';}
			...
			if ($strana!=$broj_strana) {
				$b=$strana+1;
				echo "<a href='knjige/strana/$b'><img class=\"strelica\" src=\"images/right_arrow.png\"/></a>";
			}
			else{echo '';}
			echo '</span>';
		}
