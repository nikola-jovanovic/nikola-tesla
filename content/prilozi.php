<?php
	if (isset($_GET['id'])){
		...
		if ($_COOKIE["slova"]=="cirilica") {
			echo '<h1>Прилози о Тесли</h1>';
		}
		else{
			echo '<h1>Prilozi o Tesli</h1>';
		}
		...
		foreach($articles['reports'] as $article) {
			$y = $article->get('id');
			$naslov = str_replace(' ','-',$article->get('title'));
			echo "<span class=\"naslov\"><a class=\"naslov\" href=\"prilozi/$y/$naslov\">".$article->get('title')."</a></span>";
		...
		if ($broj_strana!=1 && $broj_strana!=0){
			...
			if ($strana!=1) {
				$a=$strana-1;
				echo "<a href='prilozi/strana/$a'><img class=\"strelica\" src=\"images/left_arrow.png\"/></a>";
			}
			else{echo '';}
			...
			if ($strana!=$broj_strana) {
				$b=$strana+1;
				echo "<a href='prilozi/strana/$b'><img class=\"strelica\" src=\"images/right_arrow.png\"/></a>";
			}
			else{echo '';}
			echo '</span>';
		}
